<template>
  <div>
    <div class="page-header">
      <h2>情報分享</h2>
      <el-button type="primary" @click="openDialog(null)"><IconPlus :size="18" /> 新增文章</el-button>
    </div>
    ...
    <el-table :data="items" border stripe>
      ...
      <el-table-column prop="title" label="標題" min-width="250" show-overflow-tooltip />
      <el-table-column prop="share_date" label="日期" width="110" />
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '編輯文章' : '新增文章'" width="900px" :close-on-click-modal="false" top="5vh">
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="8">
            <el-form-item label="日期">
              <el-date-picker v-model="form.shareDate" type="date" value-format="YYYY-MM-DD" placeholder="選擇日期" style="width:100%" />
            </el-form-item>
          </el-col>
          <el-col :span="4">
...
async function loadData() {
  const res = await http.get('/share', { params: { keyword: keyword.value, page: page.value, size: size.value } })
...
function openDialog(row: any | null) {
  if (row) {
    ...
    form.value = { ...row, shareDate: row.share_date || '', isShow: row.is_show || 'Y', content: row.content || '' }
  } else {
    editingId.value = null
    form.value = { title: '', content: '', shareDate: '', image: '', isShow: 'Y' }
  }
...
async function handleSave() {
  ...
  try {
    if (editingId.value) {
      await http.put(`/share/${editingId.value}`, form.value)
    } else {
      await http.post('/share', form.value)
    }
...
async function handleDelete(row: any) {
  ...
  await http.delete(`/share/${row.id}`)
